...
(create it via `fatbeats-content-artist-context` if missing) and cite which
...
4. Stitch + trim via `fatbeats-content-gen-composite` — generations are selects,
   not finals.

## Verify + log

Preview stills → honest read → both ledgers (fatbeats-content-genmedia §3-4).
